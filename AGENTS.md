...
本文件适用于整个仓库，作为 AI 编码代理修改本项目时的唯一项目规范。除非用户另有要求，使用中文回复和说明。

## 项目状态

NeoDB Lite 处于稳定维护阶段，这是理解本仓库的最高优先级前提。

- 主要功能和架构已经基本定型，后续以小幅维护为主。
...
- 不把本文件中的版本号、Action 版本或功能描述当成永久真值；如果仓库实际文件不同，以当前文件内容为准。
...
- **NeoDB Lite**：面向 [NeoDB](https://neodb.social) 及兼容实例的非官方 Android 客户端，从零原生开发，定位为轻量的移动端收藏、标记与评价工具。
...
主源码位于 `app/src/main/java/com/krelinnbios/neodblite/`。
...
- 先读取与任务有关的实现、调用链和已有测试，不根据文件名或界面表现直接猜测根因。
- 优先沿用现有类、StateFlow、Repository 和工具函数，以最小 diff 完成明确需求。
- 修改 OAuth、host、URL、HTML 解析、评分提交或应用更新逻辑时，优先准备可复现输入并检查现有单元测试。
- UI、OAuth、网络恢复和系统安装行为即使编译或单测通过，也要提前识别仍需实机验证的部分。
- 不把一次局部修复扩展为架构清理、统一命名、全局格式化或无关依赖升级。
- 如果任务目标与现有稳定行为冲突，指出冲突和影响，再按用户明确决定实施。
...
以下内容解释当前实现为什么这样工作。除非用户明确要求改变对应行为，不应仅凭通用最佳实践将其删除或替换。
...
- **标签输入与建议**：标签输入框使用 `TextFieldValue` 管理文本和光标位置，仅以普通空格分隔标签；逗号、中文逗号、顿号和其他空白字符会作为标签内容保留。点击标签建议时替换光标所在的标签片段并去重，自动在末尾添加空格并将光标定位到末尾，方便用户连续输入多个标签。标签建议框仅在输入框获得焦点时显示。
- **指定标记日期**：编辑器默认关闭指定日期，不主动指定时省略 `created_time`，由服务端保留原时间或记录状态变更时间。输入使用数字键盘，年份满 4 位、月份满 2 位时立即显示 `-`；分隔符通过 `VisualTransformation` 显示，光标和退格按原始数字处理。数字超过 8 位、完整日期无效或晚于设备当地今天时，日期文字显示主题色及中间删除线，保存时阻止提交。指定日期须通过严格的 `YYYY-MM-DD` 校验，并提交完整 UTC 时间戳，与书架和日历直接取时间戳日期部分的口径一致；日期格式化与校验须兼容 API 24。
...
- 默认选择与现有代码一致的实现，优先范围小、风险低、可验证的修改。
- 不在 Composable 中复制网络、解析或持久化逻辑，优先复用 Repository、StateFlow 和现有工具函数。
- 不为了尚未提出的需求增加兼容层、扩展框架或未来抽象。
- 不顺手升级依赖、重排无关代码、统一整份格式或修改版本号。
- 中文文档统一使用 UTF-8，避免 PowerShell 默认编码造成乱码。
- Git diff 只包含本次任务需要的内容，不混入构建产物、设备日志、临时响应或其他无关文件。

## 文档同步

- `README.md` 面向用户，描述 NeoDB Lite 当前已经存在的能力；`AGENTS.md` 记录编码代理修改仓库时需要遵守的结构、边界和历史决策。
- 本文件是唯一的 AI 项目规范来源。
- README 主体结构维持：项目简介 → 功能概览 → 界面预览 → 使用方式 → 隐私与数据 → 内容边界 → 许可协议 → 反馈与贡献。
- README 功能列表保持 `- 四字标签：描述。` 的扁平风格，不加粗标签，不按功能再拆三级标题；标签需正好四个汉字。
- README 项目简介保留“以下说明仅描述 NeoDB Lite 当前实际提供的功能”一类范围说明。
- 简体中文、繁體中文和 English README 的事实与结构保持一致；顶部图标共用 `icon/icon.svg`。
- 修改公开功能、安装方式、隐私处理、APK 名称、发布方式或项目定位时，检查各语言 README 是否需要同步。

## 提交约定

- 一个提交聚焦一个明确主题，不把维护修复、重构、依赖升级和文档清理混在一起。
- 提交摘要和描述使用简短中文。
- 不提交构建产物、签名材料、环境文件、临时响应、抓包文件或设备日志。
- 工作区已有其他修改时，只处理并提交本次任务涉及的内容。

## 回答原则

- 把 NeoDB Lite 当作功能基本定型、仍接受小幅维护的独立项目讨论。
