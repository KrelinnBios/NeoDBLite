...
- 发现浏览：按类目（图书/电影/剧集/音乐/游戏/播客/演出）查看趋势榜，网格展示，长按条目可快速标记。
- 条目搜索：独立搜索页，跨类目或按类目搜索，分页加载，保留最近搜索历史（可点选/单删/清空），长按结果同样支持快速标记。
- 条目详情：封面、标题、评分、简介、标签（点击跳转标签搜索）、外部来源链接，展示当前账号对该条目的标记，以及公开短评/长评/笔记等社群内容。
- 标记管理：设置书架状态（想读/在读/读过/搁置，按类目显示对应动词）、0~10 评分、短评、标签、可见性（公开/仅关注者/仅自己），可选择是否同步到联邦宇宙，支持修改与删除。
- 我的书架：按状态分页查看自己的标记，可按类目或标签筛选（标签筛选跨状态展示该标签下全部条目），支持标题关键词过滤与日历视图查看每日标记分布。
- 收藏单：查看自己的收藏单列表与详情内条目，当前为只读浏览，不支持创建/编辑。
- 个人主页：头像、简介、书架统计（点击跳转对应书架）、最近完成条目、收藏单入口；设置以弹窗形式承载语言切换、主题切换、自动更新开关、手动检查更新、问题反馈、退出登录。
- 主题与语言：内置多套配色主题，支持简体中文/繁體中文/English 界面切换（非 Android 资源本地化，走应用内字符串表 `AppLanguage`/`AppStrings`）。
...
- README 结构固定为：项目简介 → 功能概览 → 界面预览 → 使用方式 → 数据与安全 → 内容边界 → 许可协议 → 反馈与贡献，与 [YamiboReaderLite](../YamiboReaderLite) 保持一致；不设「技术信息」小节。
- README 功能列表格式为 `- 四字标签：描述。`，不加粗；标签需正好四字；保持扁平列表，不按功能分组加三级标题。
- README「项目简介」结尾补一句范围说明（如"以下说明仅描述 NeoDB Lite 当前实际提供的功能"），不要只留功能段落，也不要和「数据与安全」小节重复表述。
- 顶部 logo 统一放在 `icon/logo.svg`（相对仓库根目录），四份语言版 README 共用同一份文件。
