...
- 发现浏览：按图书、电影、剧集、音乐、游戏、播客和演出查看趋势内容，网格展示，长按条目可快速标记。
- 条目搜索：独立搜索页，支持跨类目或限定类目搜索并分页加载结果，保留最近搜索历史（可点选、单条删除、一键清空），长按结果同样支持快速标记。
- 条目详情：展示封面、标题、评分、简介、标签（点击标签可跳转按标签搜索）、外部来源链接（豆瓣/IMDb/TMDB/Bangumi 等）和当前账号的标记状态。
- 社群内容：在条目详情中查看公开短评、长评、笔记等社群内容，并可跳转网页端查看更多。

### 标记与书架

- 标记编辑：支持设置想读/在读/读过/搁置等书架状态（按类目显示对应动词）、0 到 10 评分、短评、标签和可见性，可选择是否同步到联邦宇宙，也支持修改与删除标记。
- 我的书架：按书架状态分页查看自己的标记，可按类目或标签筛选（标签筛选会跨状态展示该标签下全部条目），支持按标题关键词过滤，并提供日历视图查看每日标记分布。

### 收藏单与个人主页

- 收藏清单：查看自己的收藏单列表与详情内条目（当前为只读浏览，暂不支持创建或编辑收藏单）。
- 个人主页：展示头像、简介、书架统计（点击可跳转对应书架）、最近完成条目和收藏单入口。

### 辅助设置

- 主题切换：支持多套主题配色切换。
- 语言切换：支持简体中文、繁體中文、日本語和 English 界面。
- 应用更新：启动时自动检查新版本（可在设置页关闭），支持应用内下载、多源回退、版本与签名校验、调起系统安装器、下载失败重试、手动下载入口，以及问题反馈与退出登录。
