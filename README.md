...
- 实例登录：填写 NeoDB 实例域名后，通过浏览器授权登录。
- 发现浏览：按图书、电影、剧集、音乐、游戏、播客和演出查看趋势内容，长按条目可快速标记。
- 条目搜索：支持跨类目或限定类目搜索，保留最近搜索历史，长按结果同样支持快速标记。
- 条目详情：展示封面、标题、简介、标签、外部来源链接、社群评分与点评，可设置书架状态、评分、短评、标签与可见性来标记该条目。
- 我的书架：按状态、类目、标签或标题关键词查看自己的标记，支持修改或删除，并提供日历视图。
- 个人主页：展示头像、简介、书架统计、最近完成条目和收藏单入口。
- 辅助设置：支持多套主题、三种界面语言、更新检查、问题反馈和退出登录。
