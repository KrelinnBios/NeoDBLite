# NeoDBLite

<p align="center">
  <img src="icon/icon.svg" width="128" alt="NeoDB Lite icon">
</p>

<p align="center">
  <strong>NeoDB Lite</strong><br>
  面向 NeoDB 与兼容实例的非官方 Android 标记客户端
</p>

<p align="center">
  <a href="https://github.com/KrelinnBios/NeoDBLite/releases"><img src="https://img.shields.io/github/v/release/KrelinnBios/NeoDBLite?style=flat-square&label=%E4%B8%8B%E8%BD%BD&color=2f6f73" alt="最新版本"></a>
  <img src="https://img.shields.io/badge/平台-Android%207.0%2B-247344?style=flat-square" alt="Android 7.0+">
  <img src="https://img.shields.io/badge/许可-MIT-1f5f9c?style=flat-square" alt="MIT License">
</p>

<p align="center">
  <a href="README.md">简体中文</a> ·
  <a href="README.zh-TW.md">繁體中文</a> ·
  <a href="README.en.md">English</a>
</p>

## 项目简介

NeoDB Lite 是面向 [NeoDB](https://neodb.social) 及兼容实例的非官方 Android 客户端，用于在手机上浏览、搜索和标记书影音游等条目。以下说明仅涵盖 NeoDB Lite 当前公开提供的功能。

## 功能概览

- 实例登录：填写 NeoDB 实例域名，通过浏览器完成授权登录。
- 发现浏览：按类目浏览书影音游等趋势内容，长按条目可快速标记。
- 条目搜索：支持跨类目或限定类目搜索，并保留最近搜索记录。
- 条目详情：查看封面、简介、标签、外部链接、评分与社群内容。
- 条目标记：设置书架状态、评分、短评、标签与可见性，标签仅按空格分隔，支持标签建议和指定标记日期，输入自动补全分隔符并提示无效或未来日期。
- 我的书架：按状态、类目、标签或关键词筛选，支持修改、删除和日历视图。
- 我的主页：查看个人资料、书架统计、最近完成条目，以及自己创建和收藏的合集。
- 个性设置：切换主题和界面语言，检查更新或退出登录。

## 界面预览

<p align="center">
  <img src="screenshots/Screenshot_2026-06-30-18-12-40-43_8d633091d37a6aa.jpg" width="19%" alt="发现页">
  <img src="screenshots/Screenshot_2026-06-30-18-12-53-95_8d633091d37a6aa.jpg" width="19%" alt="书架">
  <img src="screenshots/Screenshot_2026-06-30-18-13-36-45_8d633091d37a6aa.jpg" width="19%" alt="我的主页">
  <img src="screenshots/Screenshot_2026-06-30-18-14-24-14_8d633091d37a6aa.jpg" width="19%" alt="条目详情">
  <img src="screenshots/Screenshot_2026-06-30-18-17-47-14_8d633091d37a6aa.jpg" width="19%" alt="设置">
</p>

## 使用方式

### 安装使用

从 [Releases](https://github.com/KrelinnBios/NeoDBLite/releases) 下载 `NeoDB-Lite.apk` 后安装。

### 系统要求

Android 7.0（API 24）及以上。

当前 APK 仅提供 `arm64-v8a` 和 `armeabi-v7a` 架构版本。

### 更新方式

应用可在启动时或设置页检查新版本。检测到更新后，可以在应用内下载并安装，也可以前往 Releases 页面手动下载。

若系统提示无法覆盖安装，通常需要先卸载旧版，再从 Releases 重新安装。

## 隐私与数据

- 登录授权只用于访问所选择的 NeoDB 或兼容实例，登录状态保存在设备本地。
- 条目数据、封面、评分和评论来自所登录实例及其关联来源。
- 请只从本仓库 Releases 或可信来源安装 APK，避免使用来源不明的改包版本。

## 内容边界

- 本项目为非官方客户端，与 NeoDB 项目及各实例运营方无隶属关系。
- 请遵守所登录实例的服务规则、内容规范和所在地法律法规。
- 平台内容分别适用对应实例、内容提供者和权利人的条款、隐私政策及权利声明。

## 许可协议

本项目依据 [MIT License](./LICENSE) 发布，允许使用、修改、分发和商业使用，但须保留许可证与版权声明。

第三方软件、内容和外部服务不因被本项目引用、访问、编译或展示而自动纳入 MIT 许可；实例返回的数据和账户内容也不属于本项目自身代码，具体见 [THIRD-PARTY-NOTICES.md](./THIRD-PARTY-NOTICES.md)。

## 反馈与贡献

欢迎通过 [GitHub Issue](https://github.com/KrelinnBios/NeoDBLite/issues) 提交使用问题、兼容性问题、功能建议或其他改进建议。
